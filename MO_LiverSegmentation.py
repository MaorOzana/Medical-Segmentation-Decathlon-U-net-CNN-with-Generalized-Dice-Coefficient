...
def create_train_data():
    ...
    np.save('LvrImgs_train.npy', imgs)
    np.save('LvrLbls_train.npy', imgs_mask)
...
def create_test_data():
    ...
    print('-'*30)
    TestImages_data_path = os.path.join(data_path, 'Task03_liver/imagesTs')
    TestImages = os.listdir(TestImages_data_path)
    # test images file names (nii.gz - zipped)
    test_images = TestImages[70::]
    ...
    # flip between 'x' (last axis, columns) and 'z' (first axis) axis
    for index, imge in enumerate(imgs_test):
        imgst[index, :, :] = imge

    np.save('LvrImgs_test.npy', imgst)
    print('Saving to .npy files done.')
    

def load_test_data():
    imgs_test = np.load('LvrImgs_test.npy')
    return imgs_test
...
def train_and_predict():
    ...
    print('-'*30)
    model = Unet()
    model_checkpoint = ModelCheckpoint('weights_liver.h5', monitor='val_loss', save_best_only=True)
    ...
    print('-'*30)
    model.load_weights('weights_liver.h5')
    print('-'*30)
    print('Predicting masks on test data...')
    print('-'*30)
    imgs_mask_test = model.predict(imgs_test, verbose=1)
    np.save('Predicted_LvrMasks.npy', imgs_mask_test)
    print('Saving to .npy files done.')
    ...
    pred_dir = 'Predicted_Liver_Masks'
    ...
    List[73] = 1
    imageio.mimwrite('animated_liver.gif', Imgs, duration = List)
